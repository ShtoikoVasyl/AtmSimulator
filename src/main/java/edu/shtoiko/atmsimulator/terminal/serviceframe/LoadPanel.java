...
/**
 * a panel that displays an interface for loading banknotes of different
 * denominations
 */
public class LoadPanel extends JPanel {
    ...
    public LoadPanel(AvailablePanel availablePanel, StorageProvider dataWarehouseController, Currency currency,
        Messanger messanger) {
        availableBanknotes = dataWarehouseController.getResourcesByValueNames();
        setLayout(null);
        setBounds(
        ...
        for (String name : available) {
            InputPanel inputPanel = new InputPanel(
                height / (available.size() + 2) * i - ServicePersonTerminal.INDENTATIONS, "Load " + name);
        ...
        loadToMaxLeft.setFont(new Font("", Font.BOLD, MainFrame.FONT_SIZE + 5));
        ...
        loadToMaxRight.setFont(new Font("", Font.BOLD, MainFrame.FONT_SIZE + 5));
